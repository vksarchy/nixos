...
Fast capture is all under =SPC H= (one menu). Do not use =SPC n a=
(=org-agenda=) or =SPC n r n= (slow template picker).

| Keys | What |
|------+------|
| =SPC H l= | HRR lead |
| =SPC H p= | HRR property |
| =SPC H d= | HRR deal |
| =SPC H a= | Hate entry |
| =SPC H f= | Find node |
| =SPC H i= | Insert link |

Capture flow: key → type title → write free text → =C-c C-c=.
Fill drawers/fields later in the file.
